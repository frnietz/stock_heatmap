import streamlit as st
import pandas as pd
import numpy as np
import yfinance as yf
import plotly.express as px
import datetime as dt

# Set page configuration
st.set_page_config(page_title="BIST30 Stock Market Heatmap", layout="wide")

# App title and description
st.title("BIST30 Turkey Stock Market Heatmap")
st.markdown("This app displays a heatmap of BIST30 stocks based on returns over selected time intervals.")

# BIST30 components with their tickers (adding .IS suffix for Yahoo Finance)
bist30_tickers = [
    "AKBNK.IS", "ARCLK.IS", "ASELS.IS", "BIMAS.IS", "EKGYO.IS", 
    "EREGL.IS", "FROTO.IS", "GARAN.IS", "HALKB.IS", "ISCTR.IS", 
    "KCHOL.IS", "KOZAA.IS", "KOZAL.IS", "KRDMD.IS", "PETKM.IS", 
    "PGSUS.IS", "SAHOL.IS", "SASA.IS", "SISE.IS", "TAVHL.IS", 
    "TCELL.IS", "THYAO.IS", "TOASO.IS", "TSKB.IS", "TUPRS.IS", 
    "VAKBN.IS", "VESTL.IS", "YKBNK.IS", "YATAS.IS", "YKBNK.IS"
]

# Remove duplicates if any
bist30_tickers = list(set(bist30_tickers))

# Time interval selection
interval_options = {
    "1 Day": 1,
    "5 Days": 5,
    "1 Month": 30,
    "3 Months": 90,
    "6 Months": 180,
    "1 Year": 365
}

selected_interval = st.selectbox("Select Return Interval", list(interval_options.keys()))

# Function to get stock data
@st.cache_data(ttl=3600)  # Cache data for 1 hour
def get_stock_data(tickers, days):
    end_date = dt.datetime.now()
    start_date = end_date - dt.timedelta(days=days+5)  # Add buffer days
    
    # Get market data
    data = {}
    market_caps = {}
    
    for ticker in tickers:
        try:
            stock = yf.Ticker(ticker)
            hist = stock.history(start=start_date, end=end_date)
            
            if not hist.empty:
                # Get returns
                data[ticker] = hist
                
                # Get market cap
                info = stock.info
                market_caps[ticker] = info.get('marketCap', 0)
                if market_caps[ticker] == 0:
                    market_caps[ticker] = info.get('totalAssets', 1000000)  # Fallback
            
        except Exception as e:
            st.warning(f"Error fetching data for {ticker}: {e}")
    
    return data, market_caps

# Get data based on selected interval
days = interval_options[selected_interval]
with st.spinner(f"Fetching BIST30 data for {selected_interval}..."):
    stock_data, market_caps = get_stock_data(bist30_tickers, days)

# Calculate returns
returns = {}
for ticker, data in stock_data.items():
    if len(data) >= 2:
        # Calculate return for the selected period
        start_price = data['Close'].iloc[-min(len(data), days+1)]
        end_price = data['Close'].iloc[-1]
        returns[ticker] = ((end_price - start_price) / start_price) * 100

# Prepare data for heatmap
if returns:
    heatmap_data = []
    for ticker, ret in returns.items():
        # Clean ticker name for display
        display_name = ticker.replace('.IS', '')
        
        # Get market cap (use a default if not available)
        market_cap = market_caps.get(ticker, 1000000)
        
        heatmap_data.append({
            'Ticker': display_name,
            'Return (%)': round(ret, 2),
            'Market Cap': market_cap
        })
    
    df = pd.DataFrame(heatmap_data)
    
    # Create heatmap using Plotly
    fig = px.treemap(
        df,
        path=['Ticker'],
        values='Market Cap',
        color='Return (%)',
        color_continuous_scale=['red', 'white', 'green'],
        color_continuous_midpoint=0,
        title=f'BIST30 Returns Over {selected_interval} (Box Size = Market Cap)'
    )
    
    # Update hover information
    fig.update_traces(
        hovertemplate='<b>%{label}</b><br>Return: %{color:.2f}%<br>Market Cap: %{value:,.0f} TL<extra></extra>'
    )
    
    # Display the heatmap
    st.plotly_chart(fig, use_container_width=True)
    
    # Display data table
    st.subheader("BIST30 Returns Data")
    sorted_df = df.sort_values('Return (%)', ascending=False)
    st.dataframe(sorted_df, use_container_width=True)
else:
    st.error("Failed to fetch sufficient data for BIST30 stocks. Please try again later.")

# Add information footer
st.markdown("---")
st.markdown("**Note:** Market cap data and returns are fetched from Yahoo Finance. Some values might be approximate.")
st.markdown("Data is refreshed hourly. Last update: " + dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
